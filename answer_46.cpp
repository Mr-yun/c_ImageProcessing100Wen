...
Mat gaussian_filter(cv::Mat img, double sigma, int kernel_size) {
    int height = img.rows;
    int width = img.cols;
    int channel = img.channels();
...
Mat get_edge(cv::Mat fx, cv::Mat fy) {
    // get height and width
    int height = fx.rows;
    int width = fx.cols;
...
Mat histerisis(cv::Mat edge, int HT, int LT) {
...
struct_hough_table Hough_vote(struct_hough_table hought_table, Mat img) {
    int height = img.rows;
    int width = img.cols;
    int rho = 0;
    double angle = 0;
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            for (int t = 0; t < ANGLE_T; t++) {
                ...
                hought_table.table[rho + RHO_MAX][t]++;
            }
        }
    }
    return hought_table;
...
struct_hough_table Hough_NMS(struct_hough_table hough_table) {
    ...
    for (int rho = 0; rho < RHO_MAX * 2; rho++) {
        for (int t = 0; t < ANGLE_T; t++) {
            if (hough_table.table[rho][t] == 0) continue;
            if ((t - 1) >= 0 && (rho - 1) >= 0) {
...
Mat Hough_inverse(struct_hough_table hough_table, cv::Mat img){
    int height = img.rows;
    int width = img.cols;
...
int main(int argc, const char *argv[]) {
    ...
    cv::imwrite("answer46_hough.jpg", hough);
    return 0;
}